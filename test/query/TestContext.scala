...
trait TestContext extends Scope with MustThrownExpectations {
  val meta = AnalyticsMeta(
    fields = List(
      ...
      FieldDef("sumpurchases", "Sum. Purchases", Some("customers"), "sum_purchases", SelectDetails(SelectType.SingleValue)),
      FieldDef("numpurchases", "#. Purchases", Some("orders"), "Order_Id", SelectDetails(SelectType.SingleValue)),
